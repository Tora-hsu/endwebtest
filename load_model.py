...
def predict_ans(file_name):
    ...
    IATA = data_copy['IATA_Code_Marketing_Airline'].value_counts()
    value_map = dict((v, i) for i,v in enumerate(IATA.index))
    data_copy["IATA_Code_Marketing_Airline"] = data_copy.replace({"IATA_Code_Marketing_Airline":value_map})["IATA_Code_Marketing_Airline"]
    ...
    # 讀取gzip.Model
    with gzip.open('./regressor.pgz', 'r') as xgbm2:
        # x1f_new=xgbm2.Booster()
